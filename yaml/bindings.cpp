...
public:
    CosineSimilarity() {
        try {
            // Configurar el directorio de caché antes de importar módulos
            Py_SetPath(L"/app/cache");  // Opcional, si necesitas ajustar el path de Python
            py::module_ os = py::module::import("os");
            os.attr("environ")["TRANSFORMERS_CACHE"] = "/app/cache";
            os.attr("environ")["HF_HOME"] = "/app/cache";
